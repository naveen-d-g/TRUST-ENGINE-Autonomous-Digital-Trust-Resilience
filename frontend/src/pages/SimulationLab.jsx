...
const SimulationLab = () => {
  ...
  const runFullScenario = async () => {
    ...
    const scenario = [
        { type: 'SQL_INJECTION', meta: { domain: 'WEB', method: 'GET', path: '/products?id=1 OR 1=1' } },
        { type: 'BOLA_ACCESS', meta: { domain: 'API', method: 'GET', path: '/api/v1/users/123' } },
        { type: 'PORT_SCAN', meta: { domain: 'NETWORK', port: 4444, source_ip: '192.168.1.66' } },
        { type: 'PRIVILEGE_ESCALATION', meta: { domain: 'SYSTEM', user: 'www-data', command: 'sudo -i' } }
    ];

    try {
        for (const step of scenario) {
            await new Promise(r => setTimeout(r, 800)); // Visual delay
            await SimulationService.recordEvent(activeSimId, step.type, step.meta);
            // Refresh state after each step for animation
             // Actually recordEvent returns the new event, we just need to add it to state
             // But we need to do it inside the loop.
             // Wait, triggerEvent updates state. I should reuse triggerEvent logic or call it?
             // Calling triggerEvent sets processing=true which blocks re-entry.
             // I need to refactor triggerEvent or copy logic.
             // Let's refactor triggerEvent to return the result, and manage processing state in wrapper.
        }
        // Actually, let's just implement the loop here accessing the service directly 
        // and updating state to avoid state conflict with triggerEvent's processing check?
        // Better: Make helper `_record` and have both use it.
        
    } catch (err) {
        setError('Scenario playback failed');
    } finally {
        setProcessing(false);
        // Fetch final state to be sure?
        // The individual calls return data, I should update state in loop.
    }
  };
  
  // Refactored primitive for usage in both
  const _record = async (type, meta) => {
      const result = await SimulationService.recordEvent(activeSimId, type, meta);
  ...
  // Redefine triggerEvent to use _record
  const triggerEvent = async (eventType, metadata = {}) => {
    if (!activeSimId || processing) return;
    setProcessing(true);
    try {
        await _record(eventType, metadata);
    } catch (err) {
  ...
  const runKillChain = async () => {
      ...
      try {
        ...
        for (const step of scenario) {
            ...
            await _record(step.type, step.meta);
        }
      } catch (err) {
          setError('Kill Chain execution failed');
      } finally {
          setProcessing(false);
      }
  };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div className="lg:col-span-3 space-y-4">
                ...
                <div className="bg-gray-800 p-5 rounded-xl border border-gray-700">
                    ...
                    <button 
                        onClick={runKillChain}
                        disabled={processing}
                        className={`w-full p-3 mt-4 bg-red-900/20 rounded text-red-400 border border-red-900/50 transition-all flex items-center justify-center gap-2 ${processing ? 'opacity-50 cursor-not-allowed' : 'hover:bg-red-900/40 hover:text-red-300'}`}
                    >
                        <Play className="w-3 h-3 fill-current" /> Replay Kill Chain (4 Steps)
                    </button>
                </div>
            </div>
