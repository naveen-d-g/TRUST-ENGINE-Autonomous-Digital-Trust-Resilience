...
import { Shield, AlertTriangle, Activity, Lock, Globe, AlertCircle } from "lucide-react"
import { LineChart, Line, XAxis, YAxis, Tooltip, ResponsiveContainer, CartesianGrid } from "recharts"

const WebSecurityPage = () => {
  const { web } = useMonitoringStore()
  const { data: metrics, fetch } = useDashboardStore()
  
  useEffect(() => {
      fetch() // Fetch latest recommendations
  }, [fetch])

  const recommendation = metrics?.domainRecommendations?.web
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card className="border-neonBlue/20 bg-gray-900/40 backdrop-blur">
          <div className="flex justify-between items-start">
             <div>
                <div className="text-gray-400 text-xs uppercase tracking-wider">Total Requests</div>
                <div className="text-2xl font-bold text-white mt-1">{web.metrics.total}</div>
             </div>
        ...
        <Card className="border-neonOrange/20 bg-gray-900/40 backdrop-blur">
          <div className="flex justify-between items-start">
             <div>
                <div className="text-gray-400 text-xs uppercase tracking-wider">Suspicious</div>
                <div className="text-2xl font-bold text-neonOrange mt-1">{web.metrics.suspicious}</div>
             </div>
        ...
        <Card className="border-neonRed/20 bg-gray-900/40 backdrop-blur">
          <div className="flex justify-between items-start">
             <div>
                <div className="text-gray-400 text-xs uppercase tracking-wider">Escalated</div>
                <div className="text-2xl font-bold text-neonRed mt-1">{web.metrics.escalated}</div>
             </div>
        ...
        <Card className="border-neonPurple/20 bg-gray-900/40 backdrop-blur">
          <div className="flex justify-between items-start">
             <div>
                ...
                <div className="text-2xl font-bold text-neonPurple mt-1">{Math.round(web.metrics.riskAvg)}</div>
             </div>
      ...
      <Card className="border-gray-800 bg-gray-900/40 backdrop-blur overflow-hidden">
          <div className="p-4 border-b border-gray-800 flex justify-between items-center">
            <h3 className="text-white font-bold">Live Web Security Events</h3>
            <span className="text-xs font-mono text-neonBlue bg-neonBlue/10 px-2 py-1 rounded">STREAM: WEB</span>
          </div>
                        ...
                        <tr>
                            <td colSpan={3} className="px-6 py-12 text-center text-gray-500">
                                <div className="flex flex-col items-center gap-2">
                                    <div className="w-2 h-2 rounded-full bg-neonBlue animate-ping"></div>
