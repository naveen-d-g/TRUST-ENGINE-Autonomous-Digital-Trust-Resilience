...
import { Shield, AlertTriangle, Activity, Lock, Server, AlertCircle } from "lucide-react"
import { LineChart, Line, XAxis, YAxis, Tooltip, ResponsiveContainer, CartesianGrid } from "recharts"

const ApiSecurityPage = () => {
  const { api } = useMonitoringStore()
  ...
  const recommendation = metrics?.domainRecommendations?.api
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      {/* Action Required Alert */}
      {(recommendation || api.metrics.escalated > 0) && (
        <div className="bg-red-500/10 border border-red-500 rounded-lg p-4 flex items-start gap-4 animate-in fade-in slide-in-from-top-2">
            ...
            <div>
                ...
                <p className="text-gray-300 mt-1">Recommendation: <span className="text-white font-semibold">
                    {recommendation || "High Risk Detected: Investigate Escalated Events Immediately"}
                </span></p>
            </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card className="border-neonBlue/20 bg-gray-900/40 backdrop-blur">
          <div className="flex justify-between items-start">
             <div>
                <div className="text-gray-400 text-xs uppercase tracking-wider">Total Calls</div>
                <div className="text-2xl font-bold text-white mt-1">{api.metrics.total}</div>
             </div>
        ...
        <Card className="border-neonOrange/20 bg-gray-900/40 backdrop-blur">
          <div className="flex justify-between items-start">
             <div>
                <div className="text-gray-400 text-xs uppercase tracking-wider">Rate Limited</div>
                <div className="text-2xl font-bold text-neonOrange mt-1">{api.metrics.suspicious}</div>
             </div>
        ...
        <Card className="border-neonRed/20 bg-gray-900/40 backdrop-blur">
          <div className="flex justify-between items-start">
             <div>
                <div className="text-gray-400 text-xs uppercase tracking-wider">Blocked</div>
                <div className="text-2xl font-bold text-neonRed mt-1">{api.metrics.escalated}</div>
             </div>
        ...
        <Card className="border-neonPurple/20 bg-gray-900/40 backdrop-blur">
          <div className="flex justify-between items-start">
             <div>
                ...
                <div className="text-2xl font-bold text-neonPurple mt-1">{Math.round(api.metrics.riskAvg)}</div>
             </div>
      ...
      <Card className="border-gray-800 bg-gray-900/40 backdrop-blur overflow-hidden">
          <div className="p-4 border-b border-gray-800 flex justify-between items-center">
            <h3 className="text-white font-bold">Live API Security Events</h3>
            <span className="text-xs font-mono text-neonPurple bg-neonPurple/10 px-2 py-1 rounded">STREAM: API</span>
          </div>
                        ...
                        <tr>
                            <td colSpan={3} className="px-6 py-12 text-center text-gray-500">
                                <div className="flex flex-col items-center gap-2">
                                    <div className="w-2 h-2 rounded-full bg-neonPurple animate-ping"></div>
