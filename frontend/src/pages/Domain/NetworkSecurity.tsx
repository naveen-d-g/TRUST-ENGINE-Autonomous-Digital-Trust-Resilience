...
import { Shield, AlertTriangle, Activity, Lock, Network, AlertCircle } from "lucide-react"
import { LineChart, Line, XAxis, YAxis, Tooltip, ResponsiveContainer, CartesianGrid } from "recharts"

const NetworkSecurityPage = () => {
  const { network } = useMonitoringStore()
  ...
  const recommendation = metrics?.domainRecommendations?.network
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card className="border-neonBlue/20 bg-gray-900/40 backdrop-blur">
          <div className="flex justify-between items-start">
             <div>
                <div className="text-gray-400 text-xs uppercase tracking-wider">Packets</div>
                <div className="text-2xl font-bold text-white mt-1">{network.metrics.total}</div>
             </div>
        ...
        <Card className="border-neonOrange/20 bg-gray-900/40 backdrop-blur">
          <div className="flex justify-between items-start">
             <div>
                <div className="text-gray-400 text-xs uppercase tracking-wider">Port Scans</div>
                <div className="text-2xl font-bold text-neonOrange mt-1">{network.metrics.suspicious}</div>
             </div>
        ...
        <Card className="border-neonRed/20 bg-gray-900/40 backdrop-blur">
          <div className="flex justify-between items-start">
             <div>
                <div className="text-gray-400 text-xs uppercase tracking-wider">Blocked IPs</div>
                <div className="text-2xl font-bold text-neonRed mt-1">{network.metrics.escalated}</div>
             </div>
        ...
        <Card className="border-neonPurple/20 bg-gray-900/40 backdrop-blur">
          <div className="flex justify-between items-start">
             <div>
                ...
                <div className="text-2xl font-bold text-neonPurple mt-1">{Math.round(network.metrics.riskAvg)}</div>
             </div>
      ...
      <Card className="border-gray-800 bg-gray-900/40 backdrop-blur overflow-hidden">
          <div className="p-4 border-b border-gray-800 flex justify-between items-center">
            <h3 className="text-white font-bold">Live Network Events</h3>
            <span className="text-xs font-mono text-neonOrange bg-neonOrange/10 px-2 py-1 rounded">STREAM: NETWORK</span>
          </div>
                              ...
                              <td className="px-6 py-4">
                                  <div className="flex flex-col">
                                      ...
                                      <span className="text-xs font-mono text-gray-500">
                                          {`{ "src_ip": "${event.ip}", "proto": "${event.route}", "decision": "${event.decision}" }`}
                                      </span>
                                  </div>
                              </td>
                          </tr>
                        ...
                        <tr>
                            <td colSpan={3} className="px-6 py-12 text-center text-gray-500">
                                <div className="flex flex-col items-center gap-2">
                                    <div className="w-2 h-2 rounded-full bg-neonOrange animate-ping"></div>
