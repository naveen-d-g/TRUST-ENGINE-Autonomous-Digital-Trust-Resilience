import { incidentStore as useIncidentStore } from "@/store/incidentStore"
...
export const IncidentCommandPanel = ({ incidentId, status, role }: Props) => {
  const refresh = useIncidentStore(s => s.loadIncidents)
  ...
  const contain = async () => {
    try {
        ...
        notify({ type: "warning", message: "Incident Containment Protocol Initiated - Assets Isolated" })
        refresh() 
    } catch (e: unknown) {
        const error = e as Error;
        notify({ type: "critical", message: `Containment Failed: ${error.message}` })
    }
  }
  ...
  const recover = async () => {
    try {
        await new Promise(r => setTimeout(r, 1000))
        notify({ type: "success", message: "Recovery Process Started - Restoring Services" })
        refresh()
    } catch (e: unknown) {
        const error = e as Error;
        notify({ type: "critical", message: `Recovery Failed: ${error.message}` })
    }
  }
