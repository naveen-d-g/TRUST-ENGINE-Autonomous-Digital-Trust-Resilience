...
import { Clock, ShieldAlert, Zap, Filter, Search } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import { Tokens } from '../../design/tokens';
import { Transitions } from '../../design/motion';

export const Incidents: React.FC = () => {
  const { incidents, isLoading } = useSocPolling();
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-border/50 pb-6">
        <div>
            ...
            <p className="text-xs font-mono text-muted-foreground mt-1 uppercase tracking-[0.2em]">{incidents.length} Records In Buffer</p>
        </div>
        ...
        <div className="overflow-x-auto">
            ...
            <tbody className="divide-y divide-border/30">
                <AnimatePresence mode="popLayout">
                    ...
                    >
                        <td className="px-6 py-4 font-mono text-xs font-bold text-primary">{inc.incident_id}</td>
                        <td className="px-6 py-4">
                            <div className="font-bold text-foreground group-hover:text-primary transition-colors">{inc.title}</div>
                            <div className="text-[10px] text-muted-foreground uppercase font-mono mt-0.5">{inc.domain}</div>
                        </td>
                        ...
                        <td className="px-6 py-4">
                            <div className="flex flex-col gap-1">
                                {inc.targets.users.length > 0 && (
                                    <div className="text-[10px] font-mono text-muted-foreground bg-muted/50 px-2 py-0.5 rounded w-fit">
                                        USERS: {inc.targets.users.length}
                                    </div>
                                )}
                                {inc.targets.ips.length > 0 && (
                                    <div className="text-[10px] font-mono text-muted-foreground bg-muted/50 px-2 py-0.5 rounded w-fit">
                                        IP_ADDR: {inc.targets.ips.length}
                                    </div>
                                )}
                            </div>
                        </td>
                        ...
                        <td className="px-6 py-4 text-right">
                            <Link 
                                to={`/soc/incidents/${inc.incident_id}`}
