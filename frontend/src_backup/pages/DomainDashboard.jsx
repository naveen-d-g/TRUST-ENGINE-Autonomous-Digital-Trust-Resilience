...
const DomainDashboard = ({ title, domainKey }) => {
    ...
    // 1. Fetch High-Level Metrics (Polled every 5s)
    useEffect(() => {
        const fetchMetrics = async () => {
             try {
                // Assuming we add a getDomainMetrics method to metricsService or call axios direct
                // Since I didn't update api.js yet, I will inline fetch or use a generic get
                // Let's assume metricsService.getDomainMetrics exists or I'll implement it
                // Actually I should update api.js first? No, I can use axios here or just extend the pattern.
                // Let's stick to the fetch pattern used elsewhere or standard fetch.
                const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5000/api/v1'}/metrics/domain/${domainKey}`, {
    ...
    useEffect(() => {
        ...
        if (isAuthenticated) {
            // A. Fetch History Snapshot
            fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5000/api/v1'}/live/history?domain=${domainKey}`, {
                headers: { 'X-API-Key': 'dev-api-key' }
            })
            .then(res => res.json())
            .then(data => {
                if (Array.isArray(data)) {
                    // Backend sends oldest -> newest usually, we want newest at top for table, but reversed for chart?
                    // Actually, for chart we want chronological. For table we want newest first.
                    // Let's store chronological (Backend order) and reverse for Display where needed.
                    // Backend get_global_history returns deque order (oldest -> newest).
                    // So we set state as comes.
            ...
            // B. Connect Stream
            const url = `${import.meta.env.VITE_API_URL || 'http://localhost:5000/api/v1'}/live/stream?token=dev-api-key&domain=${domainKey}`;
            console.log(`Connecting to ${title} Stream...`);
