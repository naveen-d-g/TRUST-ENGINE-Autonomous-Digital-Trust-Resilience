...
const Batch = () => {
  const {
    batchFile: file, setBatchFile: setFile,
    isBatchLoading: loading, setIsBatchLoading: setLoading,
    batchResult: result, setBatchResult: setResult,
    batchError: error, setBatchError: setError
  } = useAppContext();
  ...
  const validateFile = (selectedFile) => {
    if (selectedFile && (selectedFile.type === 'text/csv' || selectedFile.name.endsWith('.csv'))) {
      ...
      setError('Invalid format. Please upload a CSV intelligence report.');
      setFile(null);
      return false;
    }
  };
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      setError(err?.response?.data?.error || 'Intelligence engine failed to process the request.');
      setLoading(false);
    }
  };
                ...
                {Object.entries(result.summary_stats?.decision_distribution || {}).map(([key, val]) => (
                  ...
                  >
                    ...
                    <div className="flex items-end justify-between mt-3">
                      <span className="text-3xl font-black text-white tracking-tighter">{val}</span>
